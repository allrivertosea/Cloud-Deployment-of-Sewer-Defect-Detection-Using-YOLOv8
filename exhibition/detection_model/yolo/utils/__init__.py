...
from detection_model import __version__
...
class IterableSimpleNamespace(SimpleNamespace):
    ...
    def __getattr__(self, attr):
        ...
        raise AttributeError(f"""
            '{name}' object has no attribute '{attr}'. This may be caused by a modified or out of date detection_model
            'default.yaml' file.\nPlease update your code with 'pip install -U detection_model' and if necessary replace
...
def get_latest_pypi_version(package_name='detection_model'):
    """
...
def get_user_config_dir(sub_dir='Dection_model'):
    """
...
def set_sentry():
    ...
    if SETTINGS['sync'] and \
            ...
            ((is_pip_package() and not is_git_dir()) or
             (get_git_origin_url() == "" and get_git_branch() == "main")):
...
def get_settings(file=USER_CONFIG_DIR / 'settings.yaml', version='0.0.2'):
    ...
    from detection_model.yolo.utils.checks import check_version
    from detection_model.yolo.utils.torch_utils import torch_distributed_zero_first
    ...
    defaults = {
        ...
        'settings_version': version}  # settings version
    ...
    with torch_distributed_zero_first(RANK):
        ...
        if not correct:
            LOGGER.warning('WARNING ⚠️ settings reset to defaults. This is normal and may be due to a '
                           'recent detection_model package update, but may have overwritten previous settings. '
...
def set_settings(kwargs, file=USER_CONFIG_DIR / 'settings.yaml'):
    """
    Function that runs on a first-time detection_model package installation to set up global settings and create necessary
    directories.
    """
