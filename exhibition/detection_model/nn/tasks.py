...
from detection_model.nn.modules import (C1, C2, C3, C3TR, SPP, SPPF, Bottleneck, BottleneckCSP, C2f, C3Ghost, C3x, Classify,
                                        Concat, Conv, ConvTranspose, Detect, DWConv, DWConvTranspose2d, Ensemble, Focus,
                                        GhostBottleneck, GhostConv, Segment)
from detection_model.yolo.utils import DEFAULT_CFG_DICT, DEFAULT_CFG_KEYS, LOGGER, colorstr, yaml_load
from detection_model.yolo.utils.checks import check_requirements, check_yaml
from detection_model.yolo.utils.torch_utils import (fuse_conv_and_bn, fuse_deconv_and_bn, initialize_weights,
...
def torch_safe_load(weight):
    ...
    """
    from detection_model.yolo.utils.downloads import attempt_download_asset
    ...
    try:
        ...
        if e.name == 'omegaconf':  # e.name is missing module name
            LOGGER.warning(f"WARNING ⚠️ {weight} requires {e.name}, which is not in  requirements."
                           f"\nAutoInstall will run now for {e.name} but this feature will be removed in the future."
                           f"\nRecommend fixes are to train a new model using updated  package or to "
                           f"download updated models from net")
        if e.name != 'models':
